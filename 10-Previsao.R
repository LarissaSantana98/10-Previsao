...
plot(modelo1, main="Previsto e Observado - AR1", plot.type="single", ylab="Data", xlab="Inflação", col=c("Blue","Black"))
...
plot(modelo2, main="Previsto e Observado AR2", plot.type="single", ylab="Data", xlab="Inflação", col=c("Red","Black"))

modeloconjunto <- data.frame(previstoAR1,previstoAR2,Inflacao)
modeloconjunto <- ts(modeloconjunto,start = 2008-01, frequency = 12)
plot(modeloconjunto, main="Previsto e Observado AR1, AR2", plot.type="single", ylab="Data", xlab="Inflação", col=c("Blue", "Red","Black"))
...
plot(modelo01, main="Previsto e Observado - MA1", plot.type="single", ylab="Data", xlab="Inflação", col=c("Green","Black"))
...
plot(modeloconjunto2, main="Previsto e Observado MA1, MA2", plot.type="single",ylab="Data", xlab="Inflação", col=c("Green", "Yellow","Black"))
...
plot(forecast(MA1, 4), main ="Previsao da Inflacao para 2017 - MA1", xlim=c(2017,2019))  #Gráfico com as previsões com um MA1
plot(forecast(MA2, 4), main ="Previsao da Inflacao para 2017 -MA2", xlim=c(2017,2019))   #Gráfico com as previsões com um MA2
